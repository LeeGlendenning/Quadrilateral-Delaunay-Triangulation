...
public class Quadrilateral {
    ...
    /*private Point[] increasePoints(Point size, Point[] verts, Point center) {
        ...
        for (Point v : verts) {
            if (v.x < center.x) {
                v.x -= (size.x - pointDifference(center, v).x);
            } else {
                v.x += (size.x - pointDifference(center, v).x);
            }
            if (v.y < center.y) {
     ...
     * @param g2d Graphics 2D object used to draw to the screen
     * @param p Point to draw quad around
     * @param scale Amount to increase quad vertices by
     * @param size Point representing x and y values to increase vertices by
     * @param pixelFactor Factor to scale pixels by
     * @param yMax Height of screen. Used to draw from bottom left corner
     */
